...
contract XNFT is ERC721, ERC721URIStorage, ERC721Burnable, IAny2EVMMessageReceiver, ReentrancyGuard, OwnerIsCreator {
    ...
    event CrossChainReceived(
        address fom, address to, uint256 tokenId, uint64 sourceChainSelector, uint64 destinationChainSelector
    );
    ...
    function crossChainTransferFrom(
        ...
        // Get the fee reuired to send the CCIP message
        ...
        if (payFeesIn == PayFeesIn.LINK) {
            ...
            // Approve the Router to transfer LINK tokens on contract's behalf.  It iwll spend the fees in LINK
        ...
        onlyEnabledSender(message.sourceChainSelector, abi.decode(message.sender, (address)))
    {
        ...
        (address from, address to, uint256 tokenId) = abi.decode(message.data, (address, address, uint256));
    ...
    function withdraw(address _beneficiary) public onlyOwner {
        ...
        if (!sent) revert FailedToWithdraw(msg.sender, _beneficiary, amount);
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721URIStorage, ERC721) returns (string memory) {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name": "CrossChain Character", "description": "A unique character NFT", "image": "', baseURI, '"}'
            )
        );
